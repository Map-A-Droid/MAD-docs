...
wget https://gist.githubusercontent.com/sn0opy/fb654915180cfbd07d5a30407c286995/raw/8467212e1371cc3f6a385e0bc7c3f63daa7a488a/rocketmap-osm-cec.sql && \
cd mad/configs/ && \
...
5. Download the Rocketmap Database Schema: https://gist.github.com/sn0opy/fb654915180cfbd07d5a30407c286995 and store it
as a file `rocketmap-osm-cec.sql`

Your directory should now look like this:
 ``` 
 --MAD-docker/
  |-- docker-compose.yml
  |-- rocketmap-osm-cec.sql
  |-- mad/
    |-- configs/
      |-- config.ini
      |-- mappings_.json
...
such as the directory `logs`, `version.json` and the whole `files` directory, which contains all calculated routes, position files and stats. As usual,
...
So e.g. `https://your-domain.com:5555` will point to port `8080` of the container, `5554` to `8000` and `5000`to `5000`.
In this case in RGC you would put `https://your-domain.com:5555` as target, in pogodroid `http://your-domain.com:5554`
...
```
db_method: rm                        # The DB scheme to use. Either "monocle" or "rm" (without the quotes)
...
### 1.3 Preparing the Database for MAD
...
This will start the `rocket-db` service.
...
Now we initialize the rocketdb database with the `rocketmap-osm-cec.sql` file we downloaded.
``` 
cat rocketmap-osm-cec.sql | docker-compose exec -T rocket-db mysql -uroot -pStrongPassword rocketdb
```

### 1.4 Start MAD
To start MAD you just execute 
``` 
