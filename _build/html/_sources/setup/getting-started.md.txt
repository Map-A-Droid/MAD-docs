...
```
Rocketmap can act as a webfrontend as well. See [webfrontends](webfrontends.md) for more informations.
...
The runtype is depending on the mode you want to use. OCR or MITM (learn more about this topic [here](mad-modes.md)). If you want to use the MITM mode, uncomment `only_scan`. For OCR start two instances, one with `only_scan` and the other one with `only_ocr` and `ocr_multitask`.  
